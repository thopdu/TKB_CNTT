  ...
  return (
    <div className="space-y-6 animate-in fade-in pb-12">
      ...
      <div className="space-y-4">
        {/* Master Weekly Timetable Selector Ribbon */}
        <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/80 shadow-xs ring-1 ring-blue-500/10">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 pb-4 mb-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[11px] flex items-center gap-1">
                    ...
                    Trung tâm điều phối duy nhất
                  </span>
                </div>
                <p className="text-xs sm:text-[13px] text-slate-500 mt-1">
                  Chọn tuần học tại đây để <strong>đồng bộ tức thì toàn bộ trang</strong>: Dashboard Giám sát, Đánh giá trùng phòng Nhà H, Thời khóa biểu toàn khóa & Phân bổ 12 phòng học.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="cursor-pointer p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition border border-slate-200"
                title="Tuần trước"
              >
              ...
              <div className="relative min-w-[210px]">
                <select
                  ...
                  className="w-full bg-blue-50/70 border border-blue-200 text-blue-950 text-xs font-bold rounded-xl py-2 px-3 pr-8 focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer shadow-2xs"
                >
                  {weeks.map((w) => (
                    <option key={w.weekId} value={w.weekId}>
                      {w.weekTitle || `Tuần ${w.weekNumber}`} {w.current ? '(Tuần Hiện Tại)' : ''} ({w.dateRange})
                    </option>
                  ))}
                </select>
          ...
          {/* Interactive Scrollable Week Cards Ribbon */}
            ...
            {weeks.map((w) => {
              ...
              return (
                <button
                  ...
                  className={`cursor-pointer shrink-0 text-left px-3.5 py-2.5 rounded-2xl border transition-all flex flex-col min-w-[140px] max-w-[170px] ${
                    isSelected
                      ? 'bg-blue-700 text-white border-blue-800 shadow-md scale-[1.02]'
                      : 'bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200'
                  }`}
                >
                  <div className="flex items-center justify-between gap-1 w-full mb-1">
                    <span className={`text-xs font-extrabold ${isSelected ? 'text-white' : 'text-slate-900'}`}>
                      {w.weekTitle || `Tuần ${w.weekNumber}`}
                    </span>
                    {w.current && (
                      <span
                  ...
                  <div className={`text-[11px] font-medium truncate ${isSelected ? 'text-blue-100' : 'text-slate-500'}`}>
                    {w.dateRange || 'Theo kế hoạch'}
                  </div>
                </button>
              );
            })}
